...
class AggregateL05RunsTest(unittest.TestCase):
    def test_any_real_pass_closes_collection(self):
        result = aggregate([
            ("run-01", {"scenario": "l05-network", "status": "BLOCKED"}),
            ("run-02", {"scenario": "l05-network", "status": "PASS"}),
        ])
    ...
    def test_all_blocked_attempts_remain_blocked(self):
        result = aggregate([
            ("run-01", {"scenario": "l05-network", "status": "BLOCKED"}),
            ("run-02", {"scenario": "l05-network", "status": "BLOCKED"}),
        ])
        self.assertEqual(result["status"], "BLOCKED")

    def test_malformed_or_failed_attempt_fails_aggregate(self):
        result = aggregate([
            ("run-01", {"scenario": "l05-network", "status": "PASS"}),
            ("run-02", {"scenario": "l05-network", "status": "FAIL", "errors": ["bad evidence"]}),
        ])
